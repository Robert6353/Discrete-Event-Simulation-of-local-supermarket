...
class clarks:
    def __init__(self, env):
       ...
       self.shop = simpy.Container(env, capacity = 15, init = 0) # No more new customers after enough people are in shop
       self.line = simpy.Container(env, capacity = 5, init = 0) #No more new customers after 20 in shop
    ...
    def run(self, env):
        while True:            
            if env.now <= 60:
                yield self.env.process(self.regular(env))

            elif env.now > 60 and env.now <= 120:
                yield self.env.process(self.prime(env))
...
shop = env.process(shop(env, clarks))

checkout = env.process(checkout(env, clarks))
        
employee_checkout = env.process(employee_numb(env, clarks))

deli_counter = env.process(deli_counter(env, clarks))
...
Revenue = (clarks.dispatch.level * random.gauss(mean_rev_per_head, std_rev_per_head) + clarks.line.level * random.gauss(mean_rev_per_head, std_rev_per_head)
           + clarks.shop.level * random.gauss(mean_rev_per_head, std_rev_per_head)) - 264
#adjust this to a normal distribution

Revenue_lost = clarks.lost.level * random.gauss(mean_rev_per_head, std_rev_per_head) + clarks.lost_food.level * random.gauss(10, std_rev_per_head)
#Adjust this to a normal distribution

f = open("base_record.txt", "a")
f.write("{}, {}, {}, {}, {}, {} \n".format(round(Revenue), round(Revenue_lost), sum(count_shop), sum(count_line), clarks.lost.level, clarks.lost_food.level))
f.close()
